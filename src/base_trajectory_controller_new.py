...
class Spline2D(object):
    ...
    def __call__(self, x,nu=0):
        return np.squeeze(np.array([F(x,nu=nu) for F in self.Fs]).T)
    ...
    def listen_loop(self):
        while not rospy.is_shutdown():
            try:
                ...
                while self.ctrl_loop_running: rospy.sleep(.001)
    ...
    def control_loop(self,joints,ts):                
        ...
        while True:
            ...
            if t_elapsed > duration+5:
                rospy.loginfo("time elapsed (+5sec)--leaving control loop")
                if plot:
                    import pylab
                    f1 = pylab.figure()
                    f2 = pylab.figure()
            ...
            # transform target from map frame to current base frame
            pos_cur = self.brett.base.get_pose("map")
            pos_targ -= pos_cur
            pos_targ[:2] = np.dot(toRotation(-pos_cur[2]), pos_targ[:2])
            ...
            if plot:
                ...
                pos_targ[2] = smaller_ang(pos_targ[2])
                pos_cur -= pos_start
                pos_cur[:2] = np.dot(toRotation(-pos_start[2]), pos_cur[:2])
