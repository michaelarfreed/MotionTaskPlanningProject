...
def recognize_random_object(obj):
    ...
    if obj.startswith("object1"):
        return "bowl"
    elif obj.startswith("object2"):
        return "mug"    
    elif obj.startswith("object3"):
        return "plate"
    else:
        raise ValueError("Unknown object type: "+obj)

# def can_stack(stacktop, newobj, env=None):
...
def can_stack(stacktop, newobj, env):
  if type(stacktop) in {str, unicode}:
    stacktop = env.GetKinBody(stacktop)
  if type(newobj) in {str, unicode}:
    newobj = env.GetKinBody(newobj)
  min_x, max_x, min_y, max_y, _ = utils.get_object_limits(stacktop)
  rad_top = (max_x - min_x + max_y - min_y) / 4
  min_x, max_x, min_y, max_y, _ = utils.get_object_limits(newobj)
  rad_new = (max_x - min_x + max_y - min_y) / 4
  if rad_new - rad_top <= 0.01:
    # need this tolerance apparently
    return True
  return False
