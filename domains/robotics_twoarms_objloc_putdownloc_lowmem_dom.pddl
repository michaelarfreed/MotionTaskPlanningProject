(define (domain robotics)
 ...
 (:action putdown
  :parameters(?o1 - object ?targetLoc - location ?robotloc - location ?gripper - manip)
...
;	       (not (clear ?target))
 ...
 (:action grasp
     ...
     :effect (and (not (at ?o1 ?l1))
		  ...
		  (not (empty ?gripper))
		  (forall (?pdp - location) (forall (?loc2 - location) (forall (?gripper2 - manip)
		  	  (when (IsAccessPointFor ?pdp ?loc2 ?gripper2)
