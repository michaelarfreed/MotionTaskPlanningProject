(define (domain robotics)
  ...
  (:types location movable manip pose count)
  ...
  (:predicates
     ...
     (Lesser ?n1 - count ?n2 - count)
     (IsTrayHeight ?n - count)
     (IsTrayLimit ?n - count)
  )
 ...
 (:action placeOnTray
   :parameters(?lrobot - pose ?obj1 - movable ?obj2 - movable  ?tray - movable  ?trayloc - location  ?gripper - manip ?num - count ?nextnum - count ?lim - count)
   :precondition(and (Object ?obj1)
		     ...
		     (Topmost ?obj2 ?tray) 
		     (smaller ?obj1 ?obj2)
         (IsTrayLimit ?lim) (IsTrayHeight ?num)
         (Next ?num ?nextnum) (Lesser ?num ?lim)
		     )
   :effect(and (not (InGripper ?obj1 ?gripper)) (OnTray ?obj1) (on ?obj1 ?obj2)
   	       (not (Topmost ?obj2 ?tray)) (Topmost ?obj1 ?tray) 
	       (empty ?gripper)
         (IsTrayHeight ?nextnum) (not (IsTrayHeight ?num))
	      )
 )	      
 ...
 (:action pickFromTray
  :parameters(?lrobot - pose  ?tray ?obj - movable  ?obj2  - movable ?trayloc  - location ?gripper - manip ?num - count ?prevnum - count)
